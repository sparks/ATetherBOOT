...
void putch(char ch)
{
	...
	tx_write_pointer = ((tx_write_pointer & 0xFF) << 8) + spi_W5100('r', S0_TX_WR1, 0x00);	
...
char getch(void)
{
	...
	while(rx_size < 0x01) {
		sock_close();
		rx_size = (spi_W5100('r', S0_RX_RSR0, 0x00) << 8) | spi_W5100('r', S0_RX_RSR1, 0x00);		
		count++;
		if (count > MAX_TIME_COUNT)
			app_start();
	}
	...
	ch = spi_W5100('r', rx_address, 0x00);
...
void delnms(unsigned int n){ 
	...
	while(n--){ 
		x=3250;       //empirically determined fudge factor 16mhz
		while(x--); 
	}
}
